...
| **Zoomed edit** | A second pass that crops toward wherever your cursor settled and scales that to 1080p — a 2x zoom at native sharpness, because the take was recorded at your display's real resolution rather than a downscale of it. Dwells become eased zoom-ins, travel zooms back out, and the plan is shown before the render so you know how many zooms it found. No clicks: global mouse-button events need a native hook we do not have, and dwell is the honest substitute. |
| **One edit, three shapes** | Widescreen, square and vertical from a single take — and the source is decoded **once** for all of them. Vertical is only worth having *because* of the cursor track: a 9:16 crop of a 16:9 screen throws away ~68% of the width, so a blind centre-crop shows you nothing. Measured against a blind crop, the aimed vertical frame is a **3.1 dB** match — i.e. an entirely different picture. |
| **Captions, locally** | Speech recognition that runs on your machine and uploads nothing — SRT and WebVTT written beside the take. Measured **word-perfect at 6.2× realtime** on CPU with the small model. This is a dependency for us rather than an architecture, because the Python runtime is already here; it is an optional ~262MB extra, and without it the app says so instead of offering a button that fails. |
