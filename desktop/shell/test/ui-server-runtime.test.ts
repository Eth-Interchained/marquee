...
const root = mkdtempSync(path.join(tmpdir(), "marquee-shell-ui-rt-"));
...
before(async () => {
  runtime = http.createServer((request, response) => {
    seen.push({
      url: request.url ?? "",
      token: request.headers["x-marquee-runtime-token"] as string | undefined,
...
test("HTTP: a caller cannot smuggle its own runtime token header", async () => {
  await fetch(`${ui.origin}/runtime/run_code`, {
    method: "POST",
    headers: { cookie, "X-Marquee-Runtime-Token": "forged", "content-type": "application/json" },
    body: "{}",
  });
