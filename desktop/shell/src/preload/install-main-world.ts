/**
 * Installs `window.marqueeShell` in the page's own world.
 *
 ...
 * `window`, so page scripts see `marqueeShell` and nothing else.
 */
...
export function installUaShellMainWorld(): boolean {
  ...
  const host = scope.__marqueeShellHost;
  if (!host) return false;
  delete scope.__marqueeShellHost;
  ...
  Object.defineProperty(window, "marqueeShell", {
    value: marqueeShell,
    writable: false,
    configurable: false,
    enumerable: true,
