...
export type ApiServerSource =
   ...
   * `accessToken` must be its MARQUEE_API_ACCESS_TOKEN, or nothing in the shell can
...
export type ShellConfig = {
   ...
   *  needs a stable MARQUEE_SESSION_BRIDGE_URL. */
  bridgePort: number;
   ...
   * there itself. `enabled: false` (MARQUEE_PY_RUNTIME=0) starts the shell without
...
export function resolveConfig(input: {
  ...
  const bundledUiDir = input.packaged
    ...
    : path.join(repoRoot, "artifacts", "studio", "dist", "public");
  ...
  const externalUi = env("MARQUEE_WORKSPACE_UI_URL");
  const externalApi = env("MARQUEE_API_SERVER_URL");
  const configuredEntry = env("MARQUEE_API_SERVER_ENTRY") ?? apiEntry;

  const workspaceUi: WorkspaceUiSource = externalUi
    ? { kind: "external", url: externalUi }
    : { kind: "bundled", dir: env("MARQUEE_WORKSPACE_UI_DIR") ?? bundledUiDir };

  const apiServer: ApiServerSource = externalApi
    ? {
        kind: "external",
        baseUrl: externalApi,
        accessToken: env("MARQUEE_API_ACCESS_TOKEN"),
      }
    : { kind: "spawn", entry: configuredEntry };

  const rawBridgePort = Number(env("MARQUEE_SHELL_BRIDGE_PORT") ?? "0");
  ...
  return {
    ...
    pythonRuntime: {
      enabled: env("MARQUEE_PY_RUNTIME") !== "0",
      backendDir:
        env("MARQUEE_PY_RUNTIME_DIR") ??
        (input.packaged
...
export function describeConfigProblem(config: ShellConfig): string | null {
  if (config.workspaceUi.kind === "bundled" && !existsSync(config.workspaceUi.dir)) {
    return [
      ...
      "or point MARQUEE_WORKSPACE_UI_URL at a running dev server.",
    ].join("\n");
  }
  ...
  if (config.apiServer.kind === "external" && !config.apiServer.accessToken) {
    return [
      `MARQUEE_API_SERVER_URL is set to ${config.apiServer.baseUrl}, but MARQUEE_API_ACCESS_TOKEN is not.`,
      "That API server receives this shell's bridge capability, so it must be gated:",
      "  - run it bound to 127.0.0.1 (HOST=127.0.0.1) with MARQUEE_API_ACCESS_TOKEN set,",
      "  - and start the shell with the same MARQUEE_API_ACCESS_TOKEN.",
  ...
  if (config.apiServer.kind === "spawn" && !existsSync(config.apiServer.entry)) {
    return [
      ...
      "or point MARQUEE_API_SERVER_URL at a running instance.",
    ].join("\n");
  }
