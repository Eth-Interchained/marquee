...
export const SHELL_COOKIE_NAME = "marquee_shell_token";
...
export const RUNTIME_TOKEN_HEADER = "X-Marquee-Runtime-Token";
...
export async function startWorkspaceUiServer(options: {
  ...
  async function proxyToApi(
    ...
    for (const [key, value] of Object.entries(request.headers)) {
      ...
      if (
        ["host", "connection", "cookie", "content-length", "x-marquee-api-token"].includes(lower)
      ) {
        continue;
      }
      headers.set(key, Array.isArray(value) ? value.join(", ") : value);
    }
    headers.set("X-Marquee-Api-Token", apiAccessToken);
